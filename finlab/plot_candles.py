...
def plot_candles(start_time, end_time, pricing, title=None,
    ...
    def series_color_function(index, technical):
        return 'g' if technical[index] < 0.5 else 'r'
    ...
    candles = ax1.bar(x, oc_max-oc_min, bottom=oc_min, color=candle_colors, linewidth=0) #畫出candle stick
    lines = ax1.vlines(x , low, high, color=candle_colors, linewidth=1)#+ 0.4 #畫出上影及下影線
    ...
    plt.xlim(left=0.5, right = max(x)+0.5)
    
    for i, t in enumerate(ticks):
        ticks[i] = t if i%space == 0 or i == len(ticks) - 1 else ''
        
    plt.xticks(x, ticks, rotation='vertical', fontsize = 14)
    ...
    for (i, technical) in enumerate(technicals):
        ...
        if i == 0:
            RF_colors = [series_color_function(j, technical) for j in x]
            ax.bar(x, technical, color = RF_colors)
            ax.hlines(0.5, xmin = min(x), xmax = max(x), linestyle = '--', color = 'red')
        else:
            ax.plot(x, technical)
            ax.hlines(0.5, xmin = min(x), xmax = max(x), linestyle = '--')
        if i < len(technicals_titles):
            ax.set_title(technicals_titles[i], loc = 'left', fontsize = 14)
